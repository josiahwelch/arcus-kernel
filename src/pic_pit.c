...
volatile uint16_t hours;  // For the clock in tooltip
char buf_str[3];

void irq_handler_c(uint32_t irq){
    if (irq == 0){                    // timer
        g_ticks++;
		if (g_ticks < 100) { // Initializer
			print_row_ak("00:00", 0, 72);
		}
		else if ((g_ticks % 6000) == 0) {
			if (minutes >= 60) minutes=0;
			cvtostr(buf_str, minutes);
			print_row_ak(buf_str, 0, 75);
			minutes++;
		}
		else if ((g_ticks % 360000) == 0) {
			if (hours >= 12) hours=0;
			cvtostr(buf_str, hours);
			print_row_ak(buf_str, 0, 72);
			hours++;
		}
    }
